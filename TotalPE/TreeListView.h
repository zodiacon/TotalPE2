...
public:
	...
	CContainedWindowT<CHeaderCtrl> m_Header;
	//
	...
	BOOL SetSubItem(HTREEITEM hItem, const LPTLVITEM pItem) {
		...
		if (pItem->iSubItem < 0 || pItem->iSubItem >= m_Header.GetItemCount()) return FALSE;
	...
	void _Init() {
		...
		m_Header.Create(this, 2, p->m_hWnd, &p->rcDefault, nullptr, m_dwHeaderStyle | HDS_FULLDRAG);
		ATLASSERT(m_Header.IsWindow());
		::SetWindowTheme(m_Header, L" ", L" ");
	...
	LPTLVITEM _GetSubItem(HTREEITEM hItem, int iSubItem) {
		ATLASSERT(hItem);
		if (iSubItem<0 || iSubItem>m_Header.GetItemCount()) return NULL;
	...
	void _CalcColumnSizes() {
		...
		m_cxHeader = 0;
		int nHeaders = m_Header.GetItemCount();
	...
	LPARAM OnHScroll(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
		...
		if (rcTree.right - rcTree.left != 0) {
			...
			m_Header.SetWindowPos(NULL, m_nOffset, 0, abs(m_nOffset) + iVSWidth + rcTree.right - rcTree.left, rcHeader.bottom - rcHeader.top, SWP_SHOWWINDOW);
		}
	...
	LRESULT OnTreeItemExpanded(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& bHandled) {
		...
		if (m_Header.GetItemCount() > 0) {
			...
			HDITEM hdr = { 0 };
			hdr.mask = HDI_WIDTH;
			m_Header.GetItem(0, &hdr);
			cx = std::max(hdr.cxy, cx);
			if (hdr.cxy != cx) {
				hdr.cxy = cx;
				m_Header.SetItem(0, &hdr);
			}
		}
		bHandled = FALSE;
		return 0;
	...
	LRESULT OnHeaderItemInsert(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
		ATLASSERT((int)wParam == m_Header.GetItemCount()); // Only supports appending right now!
		// When adding a header we need to place a TLVITEM in
		// each tree item as well...
		LRESULT lRes = m_Header.DefWindowProc(uMsg, wParam, lParam);
		if (lRes != -1) {
			LPTLVITEM pItem;
	...
	void UpdateLayout() {
		...
		if (m_Header.GetStyle() & WS_VISIBLE) {
			...
			m_Header.SetWindowPos(HWND_TOP, wpos.x, wpos.y, wpos.cx, wpos.cy, SWP_NOACTIVATE);
		}
		else {
			m_ctrlTree.SetWindowPos(HWND_TOP, &rcClient, SWP_NOACTIVATE | SWP_NOZORDER);
		}
